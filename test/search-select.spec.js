describe ("SearchSelectController", function () {
  var element, controller, scope, buildDirective;
  ...
  beforeEach(module('searchSelect', 'templates'));
  ...
  it ("should throw an exception on build if currentOption is undefined", inject(function(){
    scope.currentOption = undefined;

    expect (function(){
      buildDirective();
    })
    .toThrowError('currentOption attribute is undefined.');
  }));
  ...
  it ("should set the id key to the default value when the id key is an empty string", inject(function(){
    scope.idKey = '';
    buildDirective();
  ...
  it ("should set the search string to the options ss_display_name if an option is already selected", inject(function(){
    ...
    expect (controller.searchString).toBe(controller.currentOption.ss_display_name);
  }));
  ...
  it ("should correctly update directive variables when an option is selected", inject(function(){
    ...
    expect (controller.currentOption).toBe(selectedOption);
